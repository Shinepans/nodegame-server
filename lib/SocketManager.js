 ...
 * Obfuscating the message is necessary so that the message session field
    ...
    for (channelName in channels) {
        if (channels.hasOwnProperty(channelName)) {
            ...
            if (channel.session !== gameMsg.session) {
                gameMsg = channel.playerServer.msg.obfuscate(gameMsg);
            }
 ...
 *
 * @param {GameMsg} gameMsg The game message to send
 * @param {string} from Optional. The id or alias of the sender. Default:
...
SocketManager.prototype.send2channel = function(gameMsg, channel) {
    ...
    res = send2AllClientsInGameServer(gameMsg, channel.adminServer);
    res = res && send2AllClientsInGameServer(gameMsg, channel.playerServer);
    return res;
};
...
SocketManager.prototype.send2channelPlayers = function(gameMsg, channel) {
    ...
    return send2AllClientsInGameServer(gameMsg, channel.playerServer);
};
...
SocketManager.prototype.send2room = function(gameMsg, room) {
    ...
    if (room.channel.session !== gameMsg.session) {
        gameMsg = room.channel.playerServer.msg.obfuscate(gameMsg);
    }
 ...
 *
 * @param {GameMsg} gameMsg The game message to send
 * @param {array|PlayerList|NDDB} group The recipients of the message
 *
...
SocketManager.prototype.send2client = function(gameMsg, to) {
    ...
    if (resolvedInfo.channel.session !== gameMsg.session) {
        gameMsg = resolvedInfo.channel.playerServer.msg.obfuscate(gameMsg);
    }
...
SocketManager.prototype.broadcast2channel = function(gameMsg, channel, from) {
    ...
    if ('undefined' === typeof gameMsg._sid) {
        globalLookUp = false;
        resolvedInfo = channel.adminServer.socket.resolveClientAndSocket(
            from, globalLookUp);
        ...
        if (!resolvedInfo.success) {
            // Obfuscate.
            if (resolvedInfo.channel.session !== gameMsg.session) {
                gameMsg = resolvedInfo.channel
                    .playerServer.msg.obfuscate(gameMsg);
            }
            res = send2AllClientsInGameServer(gameMsg, channel.adminServer);
            res = res && send2AllClientsInGameServer(gameMsg,
                                                     channel.playerServer);
        }
        else {
    ...
    if ('undefined' !== typeof gameMsg._sid) {
        ...
        sid = gameMsg._sid;
        res = broadcast2AllClientsInGameServer(gameMsg,
                                               channel.adminServer,
                                               sid);
        res = res && broadcast2AllClientsInGameServer(gameMsg,
                                                      channel.playerServer,
                                                      sid);
    }
    ...
    if (!res) {
        this.sysLogger.log(
                'SocketManager.broadcast2channel: error/s occurred ' +
 ...
 *   Default: the value of the field gameMsg.from
 *
 * @return {boolean} TRUE on success
 */
SocketManager.prototype.broadcast2group = function(
        gameMsg, group, resolvedFrom) {

    var res, i, len, from, orig_id;
    res = true;
    orig_id = gameMsg.id;
 ...
 */
SocketManager.prototype.resolveClientAndSocket = function(
        gameMsg, globalLookUp) {
    ...
    // TODO: As of 4f82fa9 (may 27 2015) lookupClient returns resolvedTo
    // also for disconnected clients. Check if it is a problem.
    ...
    socket = this.clients[resolvedTo] ||
        this.server.partner.socketManager.clients[resolvedTo];
    ...
    // recipient information in all channels.
        ...
        for (channelName in channels) {
            if (channelName !== ownChannel.name &&
                ...
                socket = channel.playerServer
                    .socketManager.clients[resolvedTo] ||
                    channel.adminServer.socketManager.clients[resolvedTo];
    ...
    receiverObj = registry.getClient(resolvedTo);
    ...
    if (!room &&
        ...
        for (channelName in channels) {
            if (channelName !== ownChannel.name &&
                ...
                resolvedId = registry.lookupClient(
                        clientId, registry.getClientRoom(clientId));
...
function send2AllClientsInGameServer(gameMsg, server) {
    ...
    smoothExec = true;
    sockets = server.socketManager.sockets;
    for (i in sockets) {
