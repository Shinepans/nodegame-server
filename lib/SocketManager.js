...
var NDDB = ngc.NDDB;
var J = ngc.JSUS;
var MessagingQueue = require('./MessagingQueue.js');
...
SocketManager.prototype.send2client = function(gameMsg, to) {
    ...
    this.messagingQueue.addMessage(gameMsg);
    ...
    if (gameMsg.target === constants.target.ACK) {
        console.log("ACK for " + gameMsg.text);
    }

    //console.log("Num Msg: " + this.messagingQueue.queue.db.length);
    //console.log("Num CL Msg: " + this.messagingQueue.getAllMessagesForClient(gameMsg.to).length + ' (' + gameMsg.to + ')');
...
SocketManager.prototype.broadcast2channel = function(gameMsg, channel, from) {
    ...
    if ('undefined' === typeof gameMsg._sid) {
        globalLookUp = false;
        resolvedInfo = channel.admin.socket.resolveClientAndSocket(
                from, globalLookUp);
