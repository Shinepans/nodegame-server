...
SocketIo.prototype.attachListeners = function() {
    ...
    this.channel = this.sio.of(this.gameServer.endpoint).on('connection',
        function(socket) {
            ...
            sysLogger = that.gameServer.sysLogger;
            msgLogger = that.gameServer.msgLogger;
            ...
            if (res) {
                socket.on('message', function(msg) {
                    try {
                        ...
                        msgLogger.log('SIO_RCVD', 'info', {GameMsg: gameMsg});
                    }
                    catch(e) {
                        sysLogger.log(
                    ...
                    if (gameMsg.target === ngc.constants.target.ACK) {
                        sysLogger.log("Received ACK for " + gameMsg.text);
                        that.messagingQueue
                            .deleteMessageWithIntervalById(gameMsg.text);
                        return true;
                    }
                        ...
                        that.gameServer.socketManager.send2client(
                            ack, gameMsg.from);
                        sysLogger.log('Sent ACK for ' + gameMsg.id);
                    }
...
SocketIo.prototype.send = function(gameMsg, sid, broadcast) {
    ...
    if (sid === 'ALL') {
        ...
        msgLogger.log('SIO_SENT', 'info', {GameMsg: gameMsg});
    }
