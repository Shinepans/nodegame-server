...
GameServer.prototype.onMessage = function(msg) {
    ...
    if (Object.prototype.toString.call(msg.to) === '[object Array]') {
        ...
        var dbgStr =
            msg.toEvent() + ' ' + msg.id + ': ' + msg.from + ' -> ' + msg.to;
        console.log(dbgStr);
        this.sysLogger.log(dbgStr);
        this.emit(msg.toEvent(), msg);
    }
};
...
GameServer.prototype.handleReconnectingClient = function(
    clientId, socketId, socketObj, clientType) {
...
GameServer.prototype.attachListeners = function() {
    ...
    this.on(get + 'LANG', function(msg) {
        ...
        if (msg.to === 'SERVER') {
            ...
            that.socket.send2client(response);
        }
    });

    this.on(say + ngc.constants.target.ACK, function(msg) {
        console.log("Deleting MSG " + msg.text);
        console.log("Before: " + this.socket.messagingQueue.queue.db.length);
        this.socket.messagingQueue.deleteMessageById(msg.text);
        console.log("After: " + this.socket.messagingQueue.queue.db.length);
    });
};
